...
%         data_eKF = ft_preprocessing(cfg);
 ...
 %   file = ['pz', num2str(pz(isub)),'_uPreprodataSPC.mat'];
 %  save (file, 'hdr','evR','evCorrected','evClean','data','onsets_for_spikes', 'data_eR', 'data_eKF', 'onsets_for_eRspikes','onsets_for_eKFspikes');
%%

    load(fullfile(oripath,'data2github','SingleUnit',subjects{isub},['pz', num2str(pz(isub)),'_uPreprodataSPC.mat']))
...
%note that in this script we are defining spike onset using only micro recording
%data because macro and micro have different lenght. 
