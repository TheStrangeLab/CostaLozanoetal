...
for isub = 1;% 1:length(subjects)
    ...
    %file = ['pz', num2str(pz(isub)),'_PreprodataSPC_prestim.mat'];
    %save (file, 'hdr','evR','evCorrected','evClean','data','onsets_for_spikes', 'data_eR', 'data_eKF', 'onsets_for_eRspikes','onsets_for_eKFspikes');
    
%%
    
    load(fullfile(oripath,'data2github','SingleUnit',subjects{isub},['pz', num2str(pz(isub)),'_PreprodataSPC_prestim.mat']))
    ...
    %% plot signals
            ...
            figure
            for trl= 1:10;%size(filtered_signal,2)
            subplot(5,2,trl)
            plot(LFP_res.trial{trl}(1,[1000:5001]))
            set(gca,'XTick',1:400:4000,'XTickLabel',-0.5:0.2:1.5);
            hold on
            plot(LFP_bp.trial{trl}(1,[1000:5001]))
            set(gca,'XTick',1:400:4000,'XTickLabel',-0.5:0.2:1.5);
            hold on
            plot(filtered_signal{trl}(1,[1000:5001]))
            set(gca,'XTick',1:400:4000,'XTickLabel',-0.5:0.2:1.5);
            hold on
            plot(bandphases{trl}(1,[1000:5001]))
            legend ('lfp','lfp bp 3-12 hz','filt','phase');
            title ('trl')
            end
...
%% convert in ms
    ...
    %% data from -1 to 3
            ...
            hold on
            subplot(2,1,2)
            plot(bSpikeeKF{iwire}(trl,:),'b','LineWidth', 1)
            hold on
            plot(bandphaseseKF{trl}(1,:),'r')
            ylabel ('amygdala theta phase')
            xlabel ('time in sec')
            set(gca,'XTick',1:400:8000,'XTickLabel',-1:0.2:3);
            set(gcf,'color','w');
