...
close all
clc
% This script reproduce analysis and Supplementary Fig3a (arousal)
...
IAPS_arousal = arousal(ib,4)
IAPS_arousal_sd = arousal(ib,5)

IAPS_select_arousal = [all_nw, IAPS_arousal,IAPS_arousal_sd]
%%
...
for sub=subjects
   ...
   [what_encoded_e,ia_e,ib_e] = intersect(what_encoded,IAPS_select_arousal(1:80));
   [what_encoded_n,ia_n,ib_n] = intersect(what_encoded,IAPS_select_arousal(81:end));% 
   
    IAPS_arousal_E = IAPS_select_arousal(ib_e,2)
    IAPS_arousal_slct_E = [what_encoded_e, IAPS_arousal_E]
  
    IAPS_arousal_N = IAPS_select_arousal(80 + ib_n,2)
    IAPS_arousal_slct_N = [what_encoded_n, IAPS_arousal_N]
    ...
    e_pics_enc_ar = [e_pics_enc,IAPS_arousal_E]
    n_pics_enc_ar = [n_pics_enc,IAPS_arousal_N]
    ...
    [eR,ia_re, ib_re] = intersect(old_eCorrRemID,IAPS_arousal_slct_E)
    eR_ar = IAPS_arousal_slct_E (ib_re,2)
    eR_arousal = [eR eR_ar]
    
    old_eCorrFamID     = what_recognised(rec_onsets.eCorrFam);
    [eK,ia_ke, ib_ke] = intersect(old_eCorrFamID,IAPS_arousal_slct_E)
    eK_ar = IAPS_arousal_slct_E (ib_ke,2)
    eK_arousal = [eK eK_ar]
    
    old_eMissedID  = what_recognised(rec_onsets.eMissed);
    [eMiss,ia_misse, ib_misse] = intersect(old_eMissedID,IAPS_arousal_slct_E)
    eMiss_ar = IAPS_arousal_slct_E (ib_misse,2)
    eMIss_arousal = [eMiss eMiss_ar]
    
    old_eNotRespID = what_recognised(rec_onsets.eNotResp);
    [eNotResp,ia_eNotResp, ib_eNotResp] = intersect(old_eNotRespID,IAPS_arousal_slct_E)
    eNotResp_ar = IAPS_arousal_slct_E (ib_eNotResp,2)
    eNotResp_arousal = [eNotResp eNotResp_ar]

     old_nCorrRemID     = what_recognised(rec_onsets.nCorrRem); %gets the stimulus number, IAPS reference number of the picture
    
    [nR,ia_rn, ib_rn] = intersect(old_nCorrRemID,IAPS_arousal_slct_N)
    nR_ar = IAPS_arousal_slct_N (ib_rn,2)
    nR_arousal = [nR nR_ar]
    
    old_nCorrFamID     = what_recognised(rec_onsets.nCorrFam);
    [nK,ia_kn, ib_kn] = intersect(old_nCorrFamID,IAPS_arousal_slct_N)
    nK_ar = IAPS_arousal_slct_N (ib_kn,2)
    nK_arousal = [nK nK_ar]
    
    old_nMissedID  = what_recognised(rec_onsets.nMissed);
    [nMiss,ia_missn, ib_missn] = intersect(old_nMissedID,IAPS_arousal_slct_N)
    nMiss_ar = IAPS_arousal_slct_N (ib_missn,2)
    nMIss_arousal = [nMiss nMiss_ar]
    
    old_nNotRespID = what_recognised(rec_onsets.nNotResp);
    [nNotResp,ia_nNotResp, ib_nNotResp] = intersect(old_nNotRespID,IAPS_arousal_slct_N)
    nNotResp_ar = IAPS_arousal_slct_N (ib_nNotResp,2)
    nNotResp_arousal = [nNotResp nNotResp_ar]

% plot
marousal = [mean(eR_arousal(:,2)), mean(nR_arousal(:,2)), mean(eK_arousal(:,2)), mean(nK_arousal(:,2)), mean(eMIss_arousal(:,2)),  mean(nMIss_arousal(:,2)), mean(eNotResp_arousal(:,2)), mean(nNotResp_arousal(:,2))]
...
group_marousal(index,:) = marousal

end

%% load the data and plot the memory performance as function of arousal
clear all
close all
clc
...
load(fullfile(oripath,'data2github','Behaviour','group_arousal_c1_c2.mat'));

mat = group_marousal(:,[1:6])
figure('Position',[ 294   313   734   619],'Color','w');
ster = sterror(mat);
h = barwitherr(ster,nanmean(mat));
set(h(1),'FaceColor',[0.5 0.5 0.5]);
set(h(1),'EdgeColor',[1 1 1]);
set(gca,'XTickLabel',{'eR'; 'nR'; 'eK'; 'nK'; 'eF'; 'nF'}) 
ylabel('arousal');
hold;
...
group_marousal = group_marousal(:,[1:6]);

%save('group_arousal_c1_c2.mat', 'group_marousal');