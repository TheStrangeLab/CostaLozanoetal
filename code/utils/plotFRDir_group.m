...
%t5 = text(0.175, 0.15, ['max ', num2str(max(max([dt.FR dt.FR1])))], ...
t5 = text(0.175, 0.15, ['max ', num2str(max(dt.FR)), '/', num2str(max(dt.FR1))], ...
    'units', 'normalized', ...
    'horizontalalignment', 'right', 'verticalalignment', 'top');
% indicate number of spikes used for the analysis
t6 = text(1.05, 0.15, ['n=', num2str(dt.nspk),'/', num2str(dt.nspk1)], ...
        'units', 'normalized', ...
...
maxFR = max(dt.FR);
%maxFRdiff = max(max([dt.FR dt.FR1]))
hold on;
for iA = 1:size(dt.angleCenters, 1)
    ...
    %thisRadii   = [0; repmat(dt.FR(iA)-dt.FR1(iA), numel(thisAngles) - 2, 1); 0] ./ maxFRdiff; % normalize in relation to maximum FR
...
for iA = 1:size(dt.angleCenters, 1)
    thisAngles  = transpose([0, dt.angleCenters(iA) - deg2rad(dt.angularRes / 2):0.001:dt.angleCenters(iA) + deg2rad(dt.angularRes / 2), 0]);
    thisRadii   = [0; repmat(dt.FR1(iA), numel(thisAngles) - 2, 1); 0] ./ maxFR1; % normalize in relation to maximum FR
    %thisRadii   = [0; repmat(dt.FR(iA)-dt.FR1(iA), numel(thisAngles) - 2, 1); 0] ./ maxFRdiff; % normalize in relation to maximum FR
    [x, y]      = pol2cart(thisAngles, thisRadii);
%     patch(x, y, [0.7, 0.7, 0.7], ...
%         'FaceAlpha', 0.3, 'EdgeColor', 'none');
...
%      plot(spikeTime, std(dt.thisSpike),'Color',[0.5 0.5 0.5], 'LineWidth',0.5)
%      hold on
%      plot(spikeTime, mean(dt.thisSpike),'k', 'LineWidth',2)
%      hold off;
     
     avgw = mean(dt.thisSpike);
     stdw = std(dt.thisSpike);
     shadedErrorBar(spikeTime,avgw,stdw,'k')
