function [zpow,zmean,zstd,kInd] = ztrans_mtpow(chan,event_info,baseline_info,varargin)
...
% them.
%
% This function uses mtphasepow to calculate power and converts
% all the power values to singles to save space.  You can pass
% optional args to mtphasepow using the varargin at the end.
%
% FUNCTION:
%   zpow = ztrans_pow(chan,event_info,baseline_info,varargin)
%
% INPUT ARGS:
...
%   varargin -> See mtphasepow for optional args.
%
% OUTPUT ARGS:
...
% Changes:
%
% 9/15/05 - Added kInd functionality.
%
...
[zpow,kInd] = mtphasepow(chan,events,DurationMS,OffsetMS,BufferMS, ...
		       'powonly','usesingles',varargin{:});
zpow(zpow<=0) = eps;
zpow = log10(zpow);
...
if ~isempty(baseline_info)
  ...
  if isfield(baseline_info,'zmean') & isfield(baseline_info,'zstd')
    ...
    basepow = mtphasepow(chan,events,DurationMS,OffsetMS,BufferMS, ...
                               'powonly','usesingles',varargin{:});
