function calcPepisode(events,chan,outdir,filtFreq,freqs,resampledrate)
%CALCPEPISODE - Calculate Pepisode on entire EEG files.
%
% This function goes through all the EEG files that are specified by
% the input events structure and calculates the Pepisode transform of
% that. The output is a binary file which for each point in time
% contains a 0 if the power in that particular frequency is lower than
% baseline and a 1 if the power of that frequency is higher than the
% baseline. In order to get a P value for a certain time segment, use
% the getp_ms function
%
...
%
% FUNCTION:
%  calcPepisode(events,chan,outdir,filtFreq,freqs,resampledrate)
%
% INPUT ARGS:
...
%   resampledrate = 256     (optional), this parameter tells
%   whether to resample the eeg data
...
fprintf('Loading EEG...');
t0 = clock;
eeg = gete(chan,events,0,0,0,filtFreq,'stop',1,resampledrate);
fprintf('%g\n',etime(clock,t0));
